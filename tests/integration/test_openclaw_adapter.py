...
def _make_engine(tmp_path: Path, rules: list[GovernanceRule]) -> tuple[GovernanceEngine, FileEvidenceStore]:
...
class TestBlockScenarioIntegration:
    def test_block_with_real_yaml_rules(self, tmp_path):
        ...
        rules = _load_rules()
        engine, _ = _make_engine(tmp_path, rules)
